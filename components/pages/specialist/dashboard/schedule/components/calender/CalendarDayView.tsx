...
const CalendarDayView = ({
  ...
  const daydate = moment(getCompleteDayDate(), "DD-MM-YYYY");
  return (
    <>
      <div className="flex w-full justify-end">
        ...
        >
          {/* grid start */}
          <div className="grid grid-cols-[0px_1fr] gap-[1px] auto-rows-[minmax(30px,auto)]">
            <div className="empty-slot"></div>
            <div
              className={clsx(
                "flex w-full relative justify-center items-center bg-[#FFF] p-[8px] rounded-lg mb-[4px]",
                daydate.isSame(moment(), "day")
                  ? "bg-background-gradiant text-accent"
                  : "",
                offdays.includes(daydate.format("dddd").toLocaleLowerCase())
                  ? "bg-[#ffffff80] text-300"
                  : ""
              )}
            >
              <div className="flex w-full relative justify-center items-center rounded-lg flex-col">
                <span className="font-bold">{daydate.format("DD")}</span>
            ...
            {timeSlots.map((slot, index) => (
              <React.Fragment key={index}>
                ...
                >
                  {slot.split(" - ")[0]} {/* Display only start time */}
                </div>
                {/* this will map hour in 2 slots and return the first and 2nd hour slot */}
                {["firstHalfHour", "secondHalfHour"].map((value, i) => {
                  const inputDate = getCompleteDayDate(); // date in dd-mm-yyyy format
                  ...
                  // if slot is between booked slot
                  else if (result === "under") {
                    return null;
                  }
                  // if slot is not booked or not available
                  else {
                    ...
                    if (
                      !isSlotAvailable(inputDate, daydate.format("dddd"), slot)
                    ) {
                      return (
                        <div
                          className={clsx(
